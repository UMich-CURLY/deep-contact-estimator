...
from deep_learning_methods.utils.single_leg_fun import *
...
train_data = robot_dataset(data_path=train, Y_path=train_Y, left=(leg == 'left'), mean_path=mean,
                               std_path=std)
train_dataloader = DataLoader(dataset=train_data, batch_size=30, shuffle=True, num_workers=8, pin_memory=True)
test_data = robot_dataset(data_path=test, Y_path=test_Y, left=(leg == 'left'), mean_path=mean,
                              std_path=std)
