...
def compute_accuracy_and_loss(model, tasks_dict, dataloader, criterion):
    ...
    return num_correct_contact/num_data, correct_per_leg/num_data, num_correct_terrain/num_data, {loss_type: cur_loss_sum/len(dataloader) for loss_type, cur_loss_sum in loss_sum.items()}
...
def train(model, tasks_dict, train_dataloader, val_dataloader, config):
    ...
    for epoch in range(config['num_epoch']):
        ...
        val_acc = {}
        val_acc['contact'], val_acc_per_leg, val_acc['terrain'], val_loss_avg = compute_accuracy_and_loss(model, tasks_dict, val_dataloader, criterion)
...
def main():
    ...
    model = MultiTaskModel(tasks_dict).to(device)
